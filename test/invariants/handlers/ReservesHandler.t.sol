...
contract ReservesHandler is Test, Constants {
  ...
  function setEarmarks(uint256 serviceProviderIndex, int96 amount) public {
    ...
    if (s_totalEarmarked > 0) {
      deal(address(s_mockLink), address(s_reserves), int256(s_totalEarmarked).toUint256());
    }
  ...
  function withdraw(
    ...
    if (serviceProviderBalance > 0) {
      ...
      // make sure the contract has enough link for the withdrawal in order to avoid reverts
      deal(address(s_mockLink), address(s_reserves), toWithdraw);
