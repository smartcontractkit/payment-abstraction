...
contract AddAllowlistedAssetsUnitTest is BaseUnitTest {
  ...
  function test_applyAllowlistedAssets_RevertWhen_CallerDoesNotHaveASSET_ADMIN_ROLE()
    public
    whenCallerIsNotAssetManager
  {
    ...
    s_feeAggregatorSender.applyAllowlistedAssets(new address[](0), s_assets);
  }

  function test_applyAllowlistedAssets_RevertWhen_ContractIsPaused()
    public
  ...
  function test_applyAllowlistedAssets_RevertWhen_AnyAssetIsZeroAddress() public {
    s_assets.push(address(0));
    vm.expectRevert(Errors.InvalidZeroAddress.selector);
    s_feeAggregatorSender.applyAllowlistedAssets(new address[](0), s_assets);
  }

  function test_applyAllowlistedAssets_RevertWhen_AssetIsAlreadyAllowlisted() public {
    s_feeAggregatorSender.applyAllowlistedAssets(new address[](0), s_assets);

    vm.expectRevert(abi.encodeWithSelector(Errors.AssetAlreadyAllowlisted.selector, s_assets[0]));
    s_feeAggregatorSender.applyAllowlistedAssets(new address[](0), s_assets);
  }

  function test_applyAllowlistedAssets_SingleAsset() external {
  ...
  function test_applyAllowlistedAssets_MultipleAssets() public {
    vm.expectEmit(address(s_feeAggregatorSender));
    emit FeeAggregator.AssetAddedToAllowlist(ASSET_1);
    vm.expectEmit(address(s_feeAggregatorSender));
