...
contract FeeRouter_WithdrawNonAllowlistedAssetsIntegrationTest is BaseIntegrationTest {
  ...
  function test_withdrawNonAllowlistedAssets_RevertWhen_ContractIsPaused()
    public
    givenContractIsPaused(address(s_feeRouter))
  {
    vm.expectRevert(Pausable.EnforcedPause.selector);
    s_feeRouter.withdrawNonAllowlistedAssets(i_withdrawer, s_assetAmounts);
  }
  ...
  function test_withdrawNonAllowlistedAssets_RevertWhen_ToEqAddressZero() public {
    ...
    s_feeRouter.withdrawNonAllowlistedAssets(address(0), s_assetAmounts);
  }

  function test_withdrawNonAllowlistedAssets_RevertWhen_EmptyAssetList() public {
    vm.expectRevert(Errors.EmptyList.selector);
    s_feeRouter.withdrawNonAllowlistedAssets(i_withdrawer, new Common.AssetAmount[](0));
  }
  ...
  function test_withdrawNonAllowlistedAssets_RevertWhen_AssetIsAllowlisted() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(Errors.AssetAllowlisted.selector, s_assetAmounts[1].asset));
    s_feeRouter.withdrawNonAllowlistedAssets(i_withdrawer, s_assetAmounts);
  }
  ...
  function test_withdrawNonAllowlistedAssets_SingleAsset() public {
    ...
    emit FeeRouter.NonAllowlistedAssetWithdrawn(i_withdrawer, s_assetAmounts[0].asset, s_assetAmounts[0].amount);
  ...
  function test_withdrawNonAllowlistedAssets_MultipleAssets() public {
    ...
    emit FeeRouter.NonAllowlistedAssetWithdrawn(i_withdrawer, s_assetAmounts[0].asset, s_assetAmounts[0].amount);
    vm.expectEmit(address(s_feeRouter));
    emit FeeRouter.NonAllowlistedAssetWithdrawn(i_withdrawer, s_assetAmounts[1].asset, s_assetAmounts[1].amount);
